...
__fish_brew_complete_arg 'audit' -l fix -d 'Fix style violations automatically using RuboCop\'s auto-correct feature. When passed with `--online` for casks, also correct the `depends_on macos:` stanza, the case of artifact stanzas and the deprecated `verified` parameter'
