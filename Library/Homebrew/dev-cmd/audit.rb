    ...
    class Audit < AbstractCommand
      cmd_args do
        ...
        switch "--fix",
               description: "Fix style violations automatically using RuboCop's auto-correct feature. " \
                            ...
                            "stanza, the case of artifact stanzas and the deprecated `verified` parameter."
      ...
      def run
        ...
        if total_problems_count.positive?
          ...
          # Only fail for problems which we could not correct, so that `--fix`
          # succeeds when it has fixed everything (as `brew style --fix` does).
        ...
        annotations = formula_problems.merge(cask_problems).flat_map do |(_, path), problems|
          # Corrected problems no longer need attention.
