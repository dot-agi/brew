...
module Cask
  ...
  class Audit
    ...
    def audit_unnecessary_verified
      ...
      message = "the `verified` parameter has been deprecated; use the `url` stanza without it"

      fixed = correct_error("#{message}; removed the `verified` parameter") do |cask_ast|
    ...
    def audit_min_os
      ...
      # A cask with `on_system` blocks may declare its supported versions
      # structurally, and its minimum can differ per OS or arch, so a top-level
      # `depends_on macos:` stanza would be wrong: leave those to be fixed by hand.
      # A minimum above an existing `depends_on maximum_macos:` is unsatisfiable.
    ...
    # Corrects a problem in the cask's source file when `--fix` was passed, then
    # adds `message` as a corrected error. The block rewrites the given AST and
    # returns whether it changed anything. Returns whether the problem was
    # corrected, restoring the cask if the rewrite doesn't load.
    sig {
      params(
        message:  String,
        location: T.nilable(Homebrew::SourceLocation),
    ...
    def correct_error(message, location: nil, &_block)
      ...
      # Loading raises if the rewrite is invalid, which the `rescue` restores.
